...
public class UnorderedWindow extends WindowNode {
	...
	public UnorderedWindow(int wsize,ArrayList<? extends ProximityNode> termNodes, Index ind, RetrievalModel mod) {
		super(ind, mod);
	...
	private Set<Integer> queryTermsInWindow(ArrayList<Posting> postings, Set<Integer> unvisited, Integer start, Integer end) {
		...
		pos.add(start-1);
		for (Integer num : unvisited) {
			found = false;
			ArrayList<Integer> p = new ArrayList<>(Arrays.asList(postings.get(num).getPositionsArray()));
			for (int i = start; i < end; i++) {
				if (p.contains(i) && !pos.contains(i)) {
					found = true;
					pos.add(i);
					break;
				}
